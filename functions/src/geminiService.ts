...
export const categorizeExpenses = onCall({ secrets: [geminiApiKey] }, async (request) => {
  ...
  const { rawData } = request.data as { rawData: string };
  if (!rawData) throw new HttpsError("invalid-argument", "rawData is required.");
  ...
  try {
    ...
    if (jsonMatch) return { status: "success", data: JSON.parse(jsonMatch[0]) };
...
export const analyzeSubscriptions = onCall({ secrets: [geminiApiKey] }, async (request) => {
  ...
  const { expenses } = request.data as { expenses: any[] };
  if (!expenses) throw new HttpsError("invalid-argument", "expenses array is required.");
...
export const generateCancellationEmail = onCall({ secrets: [geminiApiKey] }, async (request) => {
  ...
  const { subscription } = request.data as { subscription: string };
  if (!subscription) throw new HttpsError("invalid-argument", "subscription name is required.");
